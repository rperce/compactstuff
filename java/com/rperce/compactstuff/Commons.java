...
public class Commons {
    ...
    public static NBTTagCompound writeStacksToNBT(NBTTagCompound data,
            ItemStack... stacks) {
        return writeStacksToNBT(data, "itemsList", stacks);
    }

    public static NBTTagCompound writeStacksToNBT(NBTTagCompound data,
            String string, ItemStack... stacks) {
        if (stacks == null) {
            System.err
                    .println("That's not a good idea, broseph.  Dem stacks null.");
            return data;
        }
    ...
    public static ItemStack[] readStacksFromNBT(NBTTagCompound data,
        ...
        if (!data.hasKey(string + "Length") || !data.hasKey(string)) {
            System.err
                    .println("Tried to read stacks from an NBTTagCompound that doesn't have stacks");
            return null;
        }
        ...
        for (int i = 0; i < itemsList.tagCount(); i++) {
            ...
            if (slot >= 0 && slot < stacks.length) stacks[slot] = stack;
        }
        return stacks;
    }

...
public class CompactTickHandler implements ITickHandler {
    ...
    public static CompactTickHandler getHandler() {
        return instance;
    }
    ...
    public void tickStart(EnumSet<TickType> type, Object... tickData) {
        for (Object playerObject : MinecraftServer
            ...
            if (playerObject instanceof EntityPlayer) {
                ...
                if (armor[1] != null) { // leggings
                    if (Ref.matches(armor[1], Ref.WOVEN_PANTS)) {
                        if (!player.isInsideOfMaterial(Material.fire))
                            player.extinguish();
    ...
    private static void doThornyThings(EntityPlayer player, double d, int dmg) {
        Collection<?> l = player.worldObj.getEntitiesWithinAABBExcludingEntity(
                player, player.boundingBox.expand(d, d, d));
        for (Object o : l) {
            if (o instanceof EntityLivingBase) {
                ...
                if (e instanceof EntityPlayer) {
                    ...
                    if (p.getLastAttacker() != null
                            && p.getLastAttacker().equals(player)) {
                        p.attackEntityFrom(
                    ...
                    if (attack != null && attack.equals(player))
                        e.attackEntityFrom(
                                DamageSource.causePlayerDamage(player), dmg);
                }
            }
        }
    }

    private static void alsoCheckForSlimes(EntityPlayer player, double d,
            int dmg) {
        Collection<?> l = player.worldObj.getEntitiesWithinAABBExcludingEntity(
                player, player.boundingBox.expand(d, d, d));
        for (Object o : l) {
            if (o instanceof EntitySlime)
                ((EntitySlime) o).attackEntityFrom(
                        DamageSource.causePlayerDamage(player), dmg);
        }
    }

    private static EntityPlayer findPlayerToAttack(EntityMob e) {
        EntityPlayer var1 = e.worldObj.getClosestVulnerablePlayerToEntity(e,
                16.0D);
        return var1 != null && e.canEntityBeSeen(var1) ? var1 : null;
    }

    /*
     * private static double distance(Entity a, Entity b) { double
     * dx=a.posX-b.posX, dy=a.posY-b.posY, dz=a.posZ-b.posZ; return
     * Math.sqrt(dx*dx+dy*dy+dz*dz); }
     */

    private static boolean setPlayerIsImmuneToFire(EntityPlayer player,
            boolean is) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    FMLLog.warning("CompactStuff could not %s fire damage.",
                            is ? "prevent" : "enable");
                    e2.printStackTrace();
                }
            }
        ...
        try {
            fireResist.setBoolean(player, is);

...
public class CompactHoe extends ItemHoe {
    ...
    public boolean onItemUse(ItemStack thisStack, EntityPlayer player,
            World world, int x, int y, int z, int these, float vars, float do_,
            float nothing) {
        StepSound sound = Block.tilledField.stepSound;
        world.playSoundEffect(x + .5d, y + .5d, z + .5d, sound.getStepSound(),
                (sound.getVolume() + 1f) / 2f, sound.getPitch() * 0.8F);
        for (int i = x - 4; i <= x + 4; i++) {
            for (int j = z - 4; j <= z + 4; j++) {
                if (world.isAirBlock(i, y + 1, j)
                        && (world.getBlockId(i, y, j) == Block.grass.blockID || world
                                .getBlockId(i, y, j) == Block.dirt.blockID)) {

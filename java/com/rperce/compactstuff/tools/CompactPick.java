...
public class CompactPick extends ItemPickaxe {
    ...
    public boolean onBlockDestroyed(ItemStack thisStack, World world,
        ...
        if (Arrays.asList(Block.oreIron.blockID, Block.oreGold.blockID,
                Block.stone.blockID, Block.netherrack.blockID,
                Block.cobblestone.blockID).contains(blockSlot)) {
            ItemStack drop = FurnaceRecipes.smelting().getSmeltingResult(
                    new ItemStack(block.idDropped(blockSlot, world.rand, 0),
                            block.quantityDropped(world.rand), block
                                    .damageDropped(world.getBlockMetadata(x, y,
                                            z))));
            drop.stackSize = (blockSlot != Block.stone.blockID
                    && world.rand.nextInt(10) < 4 ? 2 : 1);
            world.setBlockToAir(x, y, z);
            thisStack.damageItem(1, holder);
            EntityItem item = new EntityItem(world, x + .5, y + .5, z + .5,
                    drop.copy());

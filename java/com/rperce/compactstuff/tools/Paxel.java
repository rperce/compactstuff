...
public class Paxel extends ItemTool {
    ...
    public static Block[]   ores                   = { Block.oreCoal,
            Block.oreDiamond, Block.oreEmerald, Block.oreGold, Block.oreIron,
    ...
    public boolean onBlockDestroyed(ItemStack thisStack, World world,
        ...
        if (!(holder instanceof EntityPlayer) || world.getBlockId(x, y, z) == 0
        ...
        if (player.isSneaking()) {
                    ...
                    for (int c = z - 4; c <= z + 4; c++) {
                        if (world.getBlockId(r, y, c) == Block.sand.blockID) {
                            EntityItem drop = new EntityItem(
                                    ...
                                    player.posY,
                                    player.posZ,
                                    new ItemStack(Block.sand.idDropped(
                                            world.getBlockMetadata(r, y, c),
                                            rand, 0), Block.sand
                                            .quantityDropped(rand), Block.sand
                                            .damageDropped(world
                                                    .getBlockMetadata(r, y, c))));
            ...
            if (Block.blocksList[world.getBlockId(x, y, z)]
                    .getUnlocalizedName().toLowerCase().contains("ore")) {
                ...
                for (Block[] breakThese : allTheThingsToBreak) {
                    int dmg = recursivelyBreakThings(breakThese, world, x, y,
                            z, player, x, y, z);
                    if (dmg > 0) {
                        thisStack.damageItem(dmg, player);
                        return true;
    ...
    private int recursivelyBreakThings(Block[] breakThese, World world, int x,
            int y, int z, EntityPlayer player, int ox, int oy, int oz) {
        if (Math.abs(ox - x) > 8 || Math.abs(oy - y) > 8
                || Math.abs(oz - z) > 8) return 0;
        ...
        if (thisBlockID == Block.leaves.blockID) {
            EntityItem apple = new EntityItem(world, player.posX, player.posY,
                    player.posZ, new ItemStack(Item.appleRed));
            EntityItem sapling = new EntityItem(world, player.posX,
                    player.posY, player.posZ, new ItemStack(Block.sapling, 1,
                            Block.blocksList[thisBlockID].damageDropped(world
            ...
            EntityItem drop = new EntityItem(
                    ...
                    player.posZ,
                    new ItemStack(
                            Block.blocksList[thisBlockID].idDropped(
                                    world.getBlockMetadata(x, y, z), rand, 0),
        ...
        return (thisBlockID == Block.leaves.blockID ? 0 : 1)
                + recursivelyBreakThings(breakThese, world, x - 1, y, z,
                        player, ox, oy, oz)
                + recursivelyBreakThings(breakThese, world, x + 1, y, z,
                        player, ox, oy, oz)
                + recursivelyBreakThings(breakThese, world, x, y - 1, z,
                        player, ox, oy, oz)
                + recursivelyBreakThings(breakThese, world, x, y + 1, z,
                        player, ox, oy, oz)
                + recursivelyBreakThings(breakThese, world, x, y, z - 1,
                        player, ox, oy, oz)
                + recursivelyBreakThings(breakThese, world, x, y, z + 1,
                        player, ox, oy, oz);
    }

    private int recursivelyBreakOres(World world, int x, int y, int z,
            EntityPlayer player, int stop) {
        if (stop > 128) return 0;
        int thisBlockID = world.getBlockId(x, y, z);
        if (thisBlockID == 0 || thisBlockID == 1) return 0;
        if (!Block.blocksList[thisBlockID].getUnlocalizedName().toLowerCase()
                .contains("ore")) return 0;
        EntityItem drop = new EntityItem(world, player.posX, player.posY,
                player.posZ, new ItemStack(
                        Block.blocksList[thisBlockID].idDropped(
                                world.getBlockMetadata(x, y, z), rand, 0),
                        Block.blocksList[thisBlockID].quantityDropped(rand),
                        Block.blocksList[thisBlockID].damageDropped(world

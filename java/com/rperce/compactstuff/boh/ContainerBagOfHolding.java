...
public class ContainerBagOfHolding extends Container {
    ...
    public ContainerBagOfHolding(InventoryPlayer inv, InventoryBagOfHolding bag) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                this.addSlotToContainer(new Slot(this.itemBag, col + row * 9,
                        8 + col * 18, 18 + row * 18));
            }
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                this.addSlotToContainer(new Slot(this.playerInventory, col
                        + row * 9 + 9, 8 + col * 18, 32 + (row + 3) * 18));
            }
        }
        for (int col = 0; col < 9; col++) {
            this.addSlotToContainer(new Slot(this.playerInventory, col,
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < 3 * 9) {
                if (!this.mergeItemStack(temp, 3 * 9 + 1,

...
public class RenderFallingCompressed extends RenderFallingSand {
     ...
     */
    public void doRenderFalling(EntityFallingCompact entityFalling, double x,
            double y, double z) {
        GL11.glPushMatrix();
        ...
        int xx = MathHelper.floor_double(entityFalling.posX), yy = MathHelper
                .floor_double(entityFalling.posY), zz = MathHelper
                .floor_double(entityFalling.posZ), meta = entityFalling
                .getAir(); // totally where I'm storing metadata.

        if (block != null) {
            this.renderBlocks.setRenderBoundsFromBlock(block);
            this.renderBlocks.renderBlockSandFalling(block, world, xx, yy, zz,
                    meta);
        }

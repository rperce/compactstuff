...
/**
 * 4-bit metadata format: bit 3: unused bit 2: 0 -> invalid 1 -> valid bit 1,0:
 * 00 -> core 01 -> shield 10 -> frame e.g. valid core is 0101
 */
public class BlockTmog extends BlockContainer {
    ...
    public void registerIcons(IconRegister ir) {
        ...
        for (String s : new String[] { "0", "1l", "1r", "2", "3", "4t", "4b",
                "5" })
            frameicons.put(s,
                    ir.registerIcon(CSIcons.PREFIX + "tmogframe_on" + s));
        frameicons.put("!",
                ir.registerIcon(CSIcons.PREFIX + "tmogframe_on_internal"));
    }
    ...
    public static void setValid(World w, int x, int y, int z, boolean i) {
        int noo = w.getBlockMetadata(x, y, z); // pronounced like "new", a
                                               // reserved word.
        if (i)
            noo |= 4; // x0xx | 0100 = x1xx; x1xx | 0100 = x1xx
        else
            noo &= 11; // x0xx & 1011 = x0xx; x1xx & 1011 = x0xx
    ...
    public Icon getBlockTexture(IBlockAccess ba, int x, int y, int z, int s) {
        ...
        switch (s) {
            case 0:
            case 1:
                a = new boolean[] { a(ba, x, y, z - 1), a(ba, x + 1, y, z - 1),
                        a(ba, x + 1, y, z), a(ba, x + 1, y, z + 1),
                        a(ba, x, y, z + 1), a(ba, x - 1, y, z + 1),
                        a(ba, x - 1, y, z), a(ba, x - 1, y, z - 1), };
                break;
            case 3:
                a = new boolean[] { a(ba, x, y + 1, z), a(ba, x + 1, y + 1, z),
                        a(ba, x + 1, y, z), a(ba, x + 1, y - 1, z),
                        a(ba, x, y - 1, z), a(ba, x - 1, y - 1, z),
                        a(ba, x - 1, y, z), a(ba, x - 1, y + 1, z), };
                break;
            case 2:
                a = new boolean[] { a(ba, x, y + 1, z), a(ba, x - 1, y + 1, z),
                ...
                break;
            case 4:
                a = new boolean[] { a(ba, x, y + 1, z), a(ba, x, y + 1, z + 1),
                        a(ba, x, y, z + 1), a(ba, x, y - 1, z + 1),
                        a(ba, x, y - 1, z), a(ba, x, y - 1, z - 1),
                        a(ba, x, y, z - 1), a(ba, x, y + 1, z - 1), };
                break;
            case 5:
                a = new boolean[] { a(ba, x, y + 1, z), a(ba, x, y + 1, z - 1),
    ...
    private boolean a(IBlockAccess ba, int x, int y, int z) {
        return ba.getBlockId(x, y, z) == this.blockID
                && ba.getBlockMetadata(x, y, z) == 6; // 6==0110, active frame
    }

    private boolean b(IBlockAccess ba, int x, int y, int z) {
        return ba.getBlockId(x, y, z) == this.blockID
                && ba.getBlockMetadata(x, y, z) == 5; // 5==0101, active
                                                      // shielding
    }
    ...
    @Override
    public boolean isBlockSolidOnSide(World w, int x, int y, int z,
            ForgeDirection side) {
        return true;
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z,
        ...
        // placer.getHeldItem().getItemDamage(), 0x02 | 0x01);
        TileEntityTransmog tet = (TileEntityTransmog) createTileEntity(world,

...
public class CompactorGUI extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 2; c++) {
                if (CompactorRecipes.isEnabled(this.tec.enabled(),
                        this.tec.getStackInSlot(this.tec.COMFIRST + r * 2 + c))) {
                    drawTexturedModalRect(x0 + c * 18, y0 + r * 18, 176, 0, 18,
                            18);
                }
            }
        }
    ...
    protected void mouseClicked(int x, int y, int button) {
        int slot = getSlotNumberBetween(x, y, TileEntityCompactor.CRAFTFIRST,
                TileEntityCompactor.CRAFTLAST);
        if (slot == -1)
            slot = getSlotNumberBetween(x, y, TileEntityCompactor.OUTPUT,
                    TileEntityCompactor.OUTPUT);
        if (slot == -1)
            slot = getSlotNumberBetween(x, y, TileEntityCompactor.COMFIRST,
                    TileEntityCompactor.COMLAST);
        if (slot == -1)
    ...
    public void handleMouseInput() {
        ...
        int slot = getSlotNumberBetween(i, j, TileEntityCompactor.CRAFTFIRST,
                TileEntityCompactor.CRAFTLAST);
        if (slot > -1 && Mouse.getEventButton() == -1
    ...
    private boolean isMouseOverSlot(Slot slot, int x, int y) {
        return this.isPointInRegion(slot.xDisplayPosition,
                slot.yDisplayPosition, 16, 16, x, y);
    }
}

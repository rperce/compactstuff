...
public class TileEntityCompactor extends TileEntity implements ISidedInventory {
    /**
     * There are, in order, the following slots in the inventory: 3x9 inventory,
     * 3x3 crafting grid, 1 crafting result, 3x2 compacting list 27 + 9 + 1 + 6
     * = 43
     */
    ItemStack[]                   stacks = new ItemStack[27 + 9 + 1 + 6];
    // 3x9 inventory, nine crafting grid, 1 output, 6 compression. in that
    // order.
    public static final int       INVFIRST = 0, INVLAST = 26, CRAFTFIRST = 27,
            CRAFTLAST = 35, OUTPUT = 36, COMFIRST = 37, COMLAST = 42;
    private Comparator<ItemStack> sorter;
    public HashSet<ItemStack>     enabled;
    public ContainerCompactor     container;

    public synchronized HashSet<ItemStack> enabled() {
        return this.enabled;
    }
    ...
    public void onInventoryChanged() {
        ...
        for (int i = INVFIRST; i <= INVLAST; i++)
            if (getStackInSlot(i) != null) tStacks.add(getStackInSlot(i));
        ...
        for (int i = 0; i < tStacks.size() - 1; i++) {
            ...
            if (Commons.areShallowEqual(cur, tStacks.get(i + 1))) {
                int transfer = Math.min(cur.getMaxStackSize() - cur.stackSize,
                        tStacks.get(i + 1).stackSize);
        ...
        for (int i = INVFIRST; i <= INVLAST; i++) {
            if (i - INVFIRST >= tStacks.size())
                this.stacks[i] = null;
            else
                this.stacks[i] = tStacks.get(i - INVFIRST);
        }
    ...
    public int tryToMake(IRecipe r) {
        ...
        for (int o = 0; o < reqs.size(); o++) {
            ...
            for (; i <= INVLAST; i++) {
                ...
                if (stack == null) break;
                if (Commons.areShallowEqual(stack, req)
        ...
        for (int j = 0; j < indices.length; j++) {
            ...
            if (getStackInSlot(indices[j]).stackSize < 1)
                setInventorySlotContents(indices[j], null);
        }
        ...
        for (int j = INVFIRST; j <= INVLAST; j++) {
            ...
            if (getStackInSlot(j) == null) {
                ...
                break;
            } else if (Commons.areShallowEqual(cur, out)
                    && cur.getMaxStackSize() - cur.stackSize >= out.stackSize) {
    ...
    public boolean addStackToSlot(int slot, ItemStack stack) {
        ...
        if (this.stacks[slot].stackSize + stack.stackSize > stack
    ...
    @Override
    public void openChest() { /* don't do anything */
    }

    @Override
    public void closeChest() { /* don't do anything */
    }

    @Override
    public boolean isItemValidForSlot(int n, ItemStack i) {
    ...
    public boolean canExtractItem(int slot, ItemStack item, int side) {
        ...
        return !CompactorRecipes.isEnabledIngredient(this.enabled, item)
                && INVFIRST <= slot && slot <= INVLAST;
    }

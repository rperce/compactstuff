...
public class CompactorRecipes {
    ...
    static {
		...
		addBothAlternatingRecipes(ItemStuff.stack(ItemStuff.GOLD_ALLOYED),
				ItemStuff.stack(ItemStuff.GOLD_PLATE), new ItemStack(Item.glowstone),
    	...
    	for(IRecipe recipe : recipes) {
    		if(enabled.contains(recipe.getRecipeOutput())) {
    			if(recipe instanceof ShapedRecipes) {
    				...
    				for(ItemStack stack: sr.recipeItems) if(Commons.areShallowEqual(stack,input)) return true;
    			} else {
    				ShapelessRecipes sr = (ShapelessRecipes)recipe;
    				for(ItemStack stack : (List<ItemStack>)sr.recipeItems) if(Commons.areShallowEqual(stack,input)) return true;
    			}
    		}
    	} return false;
    }
    public static HashSet<IRecipe> getEnabledRecipes(HashSet<ItemStack> hashSet) {
    	...
    	for(IRecipe recipe : recipes) {
    		for(ItemStack result : hashSet) {
    			if(Commons.areShallowEqual(result,recipe.getRecipeOutput())) out.add(recipe);
    		}
    	}
    	return out;
    }
	...
	public static List<IRecipe> getMatchingRecipes(HashSet<ItemStack> enabled, ItemStack input) {
    	...
    	for(IRecipe recipe : getEnabledRecipes(enabled)) {
			if(recipe instanceof ShapedRecipes) {
				ShapedRecipes sr = (ShapedRecipes)recipe;
				for(ItemStack stack: sr.recipeItems) if(Commons.areShallowEqual(stack,input)) {
					goodOnes.add(recipe);
					break;
				}
			} else {
				ShapelessRecipes sr = (ShapelessRecipes)recipe;
				for(ItemStack stack : (List<ItemStack>)sr.recipeItems) if(Commons.areShallowEqual(stack,input)) {
					goodOnes.add(recipe);
					break;
				}
			}

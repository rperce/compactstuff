...
public class ContainerCompactor extends Container {
	...
	public ContainerCompactor(IInventory player, IInventory tileEntity) {
		...
		for(int r=0; r<3; r++)
			for(int c=0; c<9; c++)
				this.addSlotToContainer(new Slot(this.te,r*9+c,8+18*c,79+18*r));
		
		//crafting grid
		for(int r=0; r<3; r++)
			for(int c=0; c<3; c++)
				this.addSlotToContainer(new Slot(this.te,27+r*3+c, 8+18*c, 19+18*r) {
		...
		for(int r=0; r<3; r++)
			for(int c=0; c<2; c++)
				this.addSlotToContainer(new Slot(this.te,37+r*2+c, 134+18*c,19+18*r) {
					@Override public boolean getHasStack() { return false; }
					@Override public boolean canTakeStack(EntityPlayer e) { return false; }
				});
		...
		for(int r=0; r<3; r++)
			for(int c=0; c<9; c++)
				this.addSlotToContainer(new Slot(this.playerInventory, 9+r*9+c, 8+18*c, 140+18*r));
		
		//player hotbar
		for(int c=0; c<9; c++)
			this.addSlotToContainer(new Slot(this.playerInventory, c, 8+18*c, 198));		
	}
	@Override

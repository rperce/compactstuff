        ...
        if (packet.channel.equals(Metas.CH_COMPCRAFT)) {
            ...
            for (int i = tec.COMFIRST; i <= tec.COMLAST; i++) {
                ...
                if (cur == null) {
                    ...
                    break;
                } else if (cur.itemID == out.itemID
                        && cur.getItemDamage() == out.getItemDamage()) break;
            }
            ...
            if (is[4] == 999) { // shift click
                if (CompactorRecipes.isEnabled(CompactorRecipes.defaultEnabled,
                        s)) {
            ...
            ((EntityPlayer) player).worldObj.markBlockForUpdate(is[0], is[1],
                    is[2]);
        }
    }

    public TileEntityCompactor getTileEntity(int[] is, Player player) {
        TileEntity te = ((EntityPlayer) player).worldObj.getBlockTileEntity(
                is[0], is[1], is[2]);
        if (te == null || !(te instanceof TileEntityCompactor)) {
            System.out
                    .printf("No Compactor at %d, %d, %d where there should have been%n",
                            is[0], is[1], is[2]);
            return null;
        }

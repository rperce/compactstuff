...
public class BlockCompactFurnace extends BlockFurnace {
    ...
    public BlockCompactFurnace(int id) {
        super(id, false);
        setHardness(4f);
    ...
    public void registerIcons(IconRegister ir) {
        icons.put(16 * 0 + 0,
                ir.registerIcon(CSIcons.PREFIX + CSIcons.COMFURNACE_TOP));
        icons.put(16 * 0 + 1,
                ir.registerIcon(CSIcons.PREFIX + CSIcons.COMFURNACE_SIDE));
        icons.put(
                16 * 0 + 2,
                ir.registerIcon(CSIcons.PREFIX
                        + CSIcons.COMFURNACE_FRONT_INACTIVE));
        icons.put(
                16 * 0 + 3,
                ir.registerIcon(CSIcons.PREFIX
                        + CSIcons.COMFURNACE_FRONT_ACTIVE));

        icons.put(16 * 1 + 0,
                ir.registerIcon(CSIcons.PREFIX + CSIcons.CARBFURNACE_TOP));
        icons.put(16 * 1 + 1,
                ir.registerIcon(CSIcons.PREFIX + CSIcons.CARBFURNACE_SIDE));
        icons.put(
                16 * 1 + 2,
                ir.registerIcon(CSIcons.PREFIX
                        + CSIcons.CARBFURNACE_FRONT_INACTIVE));
        icons.put(
                16 * 1 + 3,
                ir.registerIcon(CSIcons.PREFIX
                        + CSIcons.CARBFURNACE_FRONT_ACTIVE));
    }
    ...
    public Icon getIcon(int side, int meta) {
        ...
        return icons.get((side == 1 || side == 0) ? blockIndex
                : (side == 3 ? blockIndex + 2 : blockIndex + 1));
    }

    public static void updateFurnace(boolean isActive, World world, int x,
            int y, int z) {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z,
        ...
        TileEntityCompactFurnace tileEntity = (TileEntityCompactFurnace) world
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (isActive(world, x, y, z)) {
            ...
            if (front == 4) {
                world.spawnParticle("smoke", dx - c, dy, dz + r, 0.0D, 0.0D,
                        0.0D);
                world.spawnParticle("flame", dx - c, dy, dz + r, 0.0D, 0.0D,
                        0.0D);
            } else if (front == 5) {
                world.spawnParticle("smoke", dx + c, dy, dz + r, 0.0D, 0.0D,
                        0.0D);
                ...
                world.spawnParticle("smoke", dx + r, dy, dz - c, 0.0D, 0.0D,
                        0.0D);
                world.spawnParticle("flame", dx + r, dy, dz - c, 0.0D, 0.0D,
                        0.0D);
            } else if (front == 3) {
                world.spawnParticle("smoke", dx + r, dy, dz + c, 0.0D, 0.0D,
                        0.0D);
    ...
    @Override
    public void breakBlock(World world, int x, int y, int z, int useless,
            int variables) {
        if (keepInv) return;
            ...
            for (int i = 0; i < te.getSizeInventory(); i++) {
                ...
                if (stack != null) {
                    ...
                    while (stack.stackSize > 0) {
                        ...
                        EntityItem drop = new EntityItem(world, x + dx, y + dy,
                                z + dz, new ItemStack(stack.itemID, randAmt,
                                        stack.getItemDamage()));

                        if (stack.hasTagCompound()) {
                            drop.getEntityItem().setTagCompound(
                                    (NBTTagCompound) stack.getTagCompound()
                                            .copy());
                        }

                        float var15 = 0.05F;
                        drop.motionX = (float) this.furnaceRand.nextGaussian()
                                * var15;
                        drop.motionY = (float) this.furnaceRand.nextGaussian()
                                * var15 + 0.2F;
                        drop.motionZ = (float) this.furnaceRand.nextGaussian()
                                * var15;
    ...
    private static char[] getMetaStr(int meta) {
        return String.format("%4s", Integer.toBinaryString(meta))
    ...
    public static void setFront(World world, int x, int y, int z, int dir) {
        ...
        world.setBlockMetadataWithNotify(x, y, z,
                Integer.parseInt(new String(metaStr), 2), 3);
    }
    ...
    public static void setIsActive(World world, int x, int y, int z, boolean act) {
        ...
        metaStr[1] = act ? '1' : '0';
        world.setBlockMetadataWithNotify(x, y, z,
                Integer.parseInt(new String(metaStr), 2), 0x1 | 0x2);
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z,
        ...
        world.setBlockMetadataWithNotify(x, y, z, placer.getHeldItem()
                .getItemDamage(), 0x04 | 0x02 | 0x01);
        int dir = MathHelper

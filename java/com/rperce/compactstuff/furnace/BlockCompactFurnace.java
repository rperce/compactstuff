...
public class BlockCompactFurnace extends BlockFurnace {
	...
	public static void setIsActive(World world,int x,int y,int z,boolean act) {
		char[] metaStr = getMetaStr(world.getBlockMetadata(x,y,z));
		metaStr[1] = act ? '1' : '0';
		world.setBlockMetadataWithNotify(x,y,z,Integer.parseInt(new String(metaStr), 2), 0x1 | 0x2);
	}
	...
	private static void setDefaultDirection(World world, int x, int y, int z) {
        ...
        if (Block.opaqueCubeLookup[var5] && !Block.opaqueCubeLookup[var6]) { dir = 2; }
        if (Block.opaqueCubeLookup[var6] && !Block.opaqueCubeLookup[var5]) { dir = 0; }
        if (Block.opaqueCubeLookup[var7] && !Block.opaqueCubeLookup[var8]) { dir = 1; }
        if (Block.opaqueCubeLookup[var8] && !Block.opaqueCubeLookup[var7]) { dir = 3; }
        setFront(world,x,y,z,dir);
    }
}

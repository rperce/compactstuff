...
public class ContainerBlazeFurnace extends Container {
	...
	public ContainerBlazeFurnace(IInventory iPlayer, IInventory iTileEntity) {
		...
		for(int r=0; r<3; r++)
			for(int c=0; c<2; c++)
				this.addSlotToContainer(new Slot(this.te,r*2+c,16+18*c,24+18*r));
		
		// output slots
		for(int r=0; r<3; r++)
			for(int c=0; c<2; c++)
				this.addSlotToContainer(new Slot(this.te,6+r*2+c,126+18*c,24+18*r));
		
		//fuel slot
		this.addSlotToContainer(new Slot(this.te,12,65,70));
		
		//player inventory
		for(int r=0; r<3; r++)
			for(int c=0; c<9; c++)
				this.addSlotToContainer(new Slot(this.player, 9+r*9+c, 8+18*c, 92+18*r));
		
		//player hotbar
		for(int c=0; c<9; c++)
			this.addSlotToContainer(new Slot(this.player, c, 8+18*c, 150));	
	}
